...
function getMarkerOrClusterPosition(deviceID) {
	...
	if (clusterer) {
		// For Leaflet MarkerCluster, use getVisibleParent to find if marker is in a cluster
		const visibleParent = clusterer.getVisibleParent(marker);
		if (visibleParent && visibleParent !== marker) {
			// Marker is clustered - return cluster position
...
async function initMap() {
	...
	// *** MAP THEME ***
	// Set our map theme using OpenStreetMap vector tiles via MapLibre GL...
	// Available styles from OpenFreeMap (free, no API key required):
	// - liberty: Standard colorful style
	// - bright: Bright, clean style
	// - positron: Light/silver style (good for data visualization)
	// - dark-matter: Dark style
	let vectorStyleUrl;
	if (mapStyle == "standard") {
		vectorStyleUrl = 'https://tiles.openfreemap.org/styles/liberty';
	} else if (mapStyle == "dark") {
		vectorStyleUrl = 'https://tiles.openfreemap.org/styles/dark';
	} else if (mapStyle == "aubergine") {
		vectorStyleUrl = 'https://tiles.openfreemap.org/styles/bright';
	} else {
		// Default: silver/positron style (light theme good for overlays)
		vectorStyleUrl = 'https://tiles.openfreemap.org/styles/positron';
	}

	// Create MapLibre GL layer with vector tiles
	...
	// Add custom controls to the map using Leaflet Control...
	const CustomControl = L.Control.extend({
		...
		onAdd: function(map) {
			...
			// Create a button for toggling visibility of the options bar...
			const weatherControls = createWeatherToggleControl(map);
			container.appendChild(weatherControls);
			...
			weatherControlButtonDiv.style.margin = "10px 0 0 0";
			...
			container.appendChild(weatherControlButtonDiv);
	...
	// Optionally create a div to hold our map tilt & heading buttons (note: Leaflet doesn't support tilt/heading)...
	if (showMapTiltControls) {
		const TiltControl = L.Control.extend({
			options: { position: 'bottomright' },
			onAdd: function(map) {
				const mapControlButtonDiv = L.DomUtil.create('div', 'leaflet-bar leaflet-control');
				...
				// Note: Leaflet doesn't support tilt/heading, but we keep the controls for UI consistency
				...
				L.DomEvent.disableClickPropagation(mapControlButtonDiv);
				return mapControlButtonDiv;
			}
		});
		new TiltControl().addTo(map);
	};
...
function createUpdateArea(map) {
	const UpdateAreaControl = L.Control.extend({
		options: { position: 'topleft' },
		onAdd: function(map) {
			const updateAreaDiv = L.DomUtil.create('div', 'leaflet-bar leaflet-control');
	...
	new UpdateAreaControl().addTo(map);
	return document.getElementById("refreshStatusArea");
}

function adjustMap(mode, amount) {
	// Note: Leaflet doesn't support tilt/heading, so we just update the variables for UI consistency
	switch (mode) {
		case "tilt":
			mapTilt = Math.max(0, Math.min(67.5, mapTilt + amount));
			break;
		case "rotate":
			mapHeading = (mapHeading + amount + 360) % 360;
			break;
		default:
			break;
	};
		...
		groupData.forEach((thisItem) => {
			...
			async function plotMarker(thisItem, lat, lng, address) {
				if (address != null) {
					...
					let marker = L.marker([cachedAddresses[groupID].lat, cachedAddresses[groupID].lng], {
						icon: markerIcon,
						title: thisItem.name,
					...
					// Store content reference for clustering popup
					marker.content = content;
					// Store severity for clustering
					marker.zIndex = pinIndex;
				...
				if (itemsProcessed == totalGroups) {
					...
					if (!disableClustering) {
						...
						// Initialize Leaflet marker clusters with custom icon creator...
						clusterer = L.markerClusterGroup({
							...
							iconCreateFunction: function(cluster) {
								...
								childMarkers.forEach(m => {
									const severity = m.zIndex > 0 ? m.zIndex.toString() : "0";
									severityCounts.set(severity, (severityCounts.get(severity) || 0) + 1);
								});

								// Calculate percentages and offsets for donut chart
								...
								severityPercents.forEach((value, key) => {
									...
									if (percent > 0) {
										if (offsetSum == 0) {
											...
											severityOffsets.set(key, (100 - offsetSum) + baseOffset);
										}
								...
								return L.divIcon({
									html: `<div style="position: relative; width: 45px; height: 45px;">
										<img src="data:image/svg+xml;base64,${svg}" style="width: 45px; height: 45px;" />
										<div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-size: 12px; font-weight: bold; color: rgba(0,0,0,0.9);">${count}</div>
									</div>`,
						...
						// Handle cluster click to show popup with details
						clusterer.on('clusterclick', function(e) {
							...
							const deviceDetails = childMarkers.map(marker => {
								...
								let sdtStatus = '';
								let statusRank = 0;
								if (iconDiv.classList.contains('critical')) { status = 'critical'; statusRank = 4; }
								else if (iconDiv.classList.contains('error')) { status = 'error'; statusRank = 3; }
								else if (iconDiv.classList.contains('warn')) { status = 'warning'; statusRank = 2; }
								if (iconDiv.classList.contains('sdt')) { sdtStatus = ' (SDT)'; status = 'sdt'; statusRank = 1; }
								return { name: nameLink.textContent, link: nameLink.href, status, sdtStatus, statusRank, latlng: marker.getLatLng() };
							}).sort((a,b) => b.statusRank - a.statusRank || a.name.localeCompare(b.name));

							const clusterBounds = L.latLngBounds(deviceDetails.map(d => d.latlng));
							const boundsStr = `[[${clusterBounds.getSouth()},${clusterBounds.getWest()}],[${clusterBounds.getNorth()},${clusterBounds.getEast()}]]`;

							const popupContent = `
								<div class="mapInfoPopupWindow">
									<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
										<div style="font-weight: 600; font-size: 14px;">Cluster Summary (${childMarkers.length})</div>
										<button onclick="map.fitBounds(${boundsStr})" style="padding: 6px 12px; background: #1a73e8; color: white; border: none; border-radius: 4px; font-size: 12px; cursor: pointer;">Zoom</button>
									</div>
									<div style="max-height: 250px; overflow-y: auto;">
										${deviceDetails.map(d => `<div style="padding: 4px 0; border-bottom: 1px solid #eee;"><a href="${d.link}" target="_blank" style="color: #1a73e8;">${d.name}</a> <span style="color: #666;">(${d.status}${d.sdtStatus})</span></div>`).join('')}
									</div>
								</div>`;

							if (clusterInfoWindow) { map.closePopup(clusterInfoWindow); }
							clusterInfoWindow = L.popup().setLatLng(cluster.getLatLng()).setContent(popupContent).openOn(map);
						});
					...
					// Note: Leaflet doesn't support tilt/heading, so we skip these calls
				};
			};
		});
	};
...
async function plotConnection(connection) {
	...
	if (instanceData) {
		...
		const leafletCoords = tmpCoords.map(c => [c.lat, c.lng]);
		const thisPath = L.polyline(leafletCoords, {
		...
		let linePopup = L.popup({ closeButton: false });

		thisPath.on("mouseover", function(e) {
			linePopup.setLatLng(e.latlng);
			linePopup.setContent(`<strong>${connection.connectionName}</strong><br/>Connection alert status: <a href="/santaba/uiv4/resources/treeNodes/t-i,id-${connection.instanceID}?source=details&tab=alert" target="_blank" title="Click to view alerts" style="border: 0;">${alertStatus}</a>`);
			linePopup.openOn(map);
		});
		thisPath.on("mouseout", function(e) {
			map.closePopup(linePopup);
		});
...
function toggleHighlight(markerView, group) {
	...
	// For Leaflet markers, we need to access the icon element differently
	const iconElement = markerView.getElement();
	if (!iconElement) return;
	const contentDiv = iconElement.querySelector('.group');
	if (!contentDiv) return;

	if (contentDiv.classList.contains("highlight")) {
		contentDiv.classList.remove("highlight");
		markerView.setZIndexOffset(Number(contentDiv.dataset.severity) || 0);
	} else {
		let foo = document.querySelectorAll('.highlight');
		if (foo.length > 0) {
			foo[0].classList.remove("highlight");
		};
		markerView.setZIndexOffset(1000);
...
/*
// OLD GOOGLE MAPS RENDERER - Commented out since we're now using Leaflet MarkerCluster with iconCreateFunction
...
function enableWeather() {
	...
	if (weatherCheckbox.checked) {
		...
		if (weatherGeoJsonLayer) {
			map.removeLayer(weatherGeoJsonLayer);
			weatherGeoJsonLayer = null;
		}
...
async function addWeatherLayer() {
	...
	if (weatherCheckbox.checked) {
		...
		// Remove any previous tile overlay...
		if (weatherTileLayer) {
			map.removeLayer(weatherTileLayer);
			weatherTileLayer = null;
		}
		...
		if (mapType == "radar") {
			...
			// Create the Leaflet tile layer...
			weatherTileLayer = L.tileLayer(rvAPIData.host + rvMapFrames[rvLastPastFramePosition].path + '/256/{z}/{x}/{y}/' + colorScheme + '/' + smooth + '_' + snow + '.png', {
		...
		// Show NEXRAD layers if appropriate...
			...
			// Create the Leaflet tile layer...
			weatherTileLayer = L.tileLayer('http://mesonet.agron.iastate.edu/cache/tile.py/1.0.0/' + mapType + '/{z}/{x}/{y}.png?' + Date.now(), {
				opacity: weatherOpacity,
				attribution: 'NEXRAD data &copy; <a href="https://mesonet.agron.iastate.edu/">Iowa State Mesonet</a>'
			});
			weatherTileLayer.addTo(map);
		...
		// Show OpenWeather layers if appropriate...
			...
			// Create the Leaflet tile layer...
			weatherTileLayer = L.tileLayer('https://tile.openweathermap.org/map/' + mapType + '/{z}/{x}/{y}.png?appid=' + openWeatherMapsAPIKey, {
				opacity: weatherOpacity,
				maxZoom: 9,
			...
			// Remove any previous GeoJSON overlay...
		if (weatherGeoJsonLayer) {
			map.removeLayer(weatherGeoJsonLayer);
			weatherGeoJsonLayer = null;
		}
		...
		if (optionalMapType == "us-fires") {
			try {
				...
				const response = await fetch(`https://services9.arcgis.com/RHVPKKiFTONKtxq3/arcgis/rest/services/USA_Wildfires_v1/FeatureServer/1/query?where=CurrentDateAge+<%3D+7&outFields=*&f=geojson&ts=${Date.now()}`);
				if (!response.ok) throw new Error(`Wildfire API error: ${response.status}`);
				const data = await response.json();

				// Create Leaflet GeoJSON layer with styling and popups...
				weatherGeoJsonLayer = L.geoJSON(data, {
					style: { fillColor: 'red', color: 'salmon', weight: 1, fillOpacity: 0.5 },
					onEachFeature: function(feature, layer) {
						const props = feature.properties;
						let comments = props.Comments || "(comments not available)";
						let acres = props.GISAcres ? Number(props.GISAcres).toLocaleString('en-US', numFormatOptions) : "(not available)";
						let fireCategory = props.FeatureCategory || "(not available)";

						const popupContent = `<div style="line-height:1.35;overflow:hidden;white-space:nowrap;color:black;">
							<span style="font-weight:700;">Wildfire &quot;${props.IncidentName}&quot;</span><br/>
							${comments}<br/><br/>
							Calculated Acres: ${acres}<br/>
							Category: ${fireCategory}<br/>
							Days Since Last GIS Update: ${props.CurrentDateAge}<br/>
							GIS Map Method: ${props.MapMethod}
						</div>`;
		...
		// Look to see if we should add power outages to the map...
			...
			try {
				...
				const countResponse = await fetch(`${baseUrl}?where=0%3D0&outFields=*&returnCountOnly=true&f=pjson&ts=${Date.now()}`);
				if (!countResponse.ok) {
				...
				for (let currentPage = 0; currentPage <= pageCount; currentPage++) {
					...
					const pageUrl = `${baseUrl}?where=0%3D0&outFields=*&resultOffset=${offset}&resultRecordCount=${maxPerPage}&f=pgeojson&ts=${Date.now()}`;
					pagePromises.push(
				...
				// Create Leaflet GeoJSON layer with dynamic styling...
				weatherGeoJsonLayer = L.geoJSON(geoJSON, {
					style: function(feature) {
						let percentAffected = 0;
						if (feature.properties.TrackedCount > 0) {
							percentAffected = feature.properties.OutageCount / feature.properties.TrackedCount;
						}
						return {
							fillColor: 'red',
					...
					onEachFeature: function(feature, layer) {
						const props = feature.properties;
						const timestamp = new Date(props.LastUpdate);
						let percentAffected = 0;
						if (props.TrackedCount > 0) {
							percentAffected = props.OutageCount / props.TrackedCount * 100;
						}
					...
					const popupContent = `<div style="line-height:1.5;overflow:hidden;white-space:nowrap;color:#333;">
							<h3 style="margin:0;">${props.NAME} County Power Outages</h3>
							${donutChart}
							<p style="padding:5px 0;">Power Customers Affected: <strong>${Math.round(percentAffected)}%</strong>
							(${props.OutageCount.toLocaleString()} of ${props.TrackedCount.toLocaleString()})</p>
						...
						layer.on('mouseover', function(e) {
							e.target.setStyle({ weight: 3, opacity: 0.5 });
						});
						layer.on('mouseout', function(e) {
							weatherGeoJsonLayer.resetStyle(e.target);
						});
					}
				});
				weatherGeoJsonLayer.addTo(map);
			...
			try {
				// Load the earthquake data from the USGS site...
				const quakeUrl = quakeMode == "time"
				...
				if (!response.ok) throw new Error(`Earthquake API error: ${response.status}`);
				const data = await response.json();
				console.debug("Earthquake data loaded");

				// Create Leaflet GeoJSON layer with point-to-layer for custom markers...
				weatherGeoJsonLayer = L.geoJSON(data, {
					pointToLayer: function(feature, latlng) {
						...
						if (quakeMode == "time") {
							...
							if (timeScale > iconOpacity) iconOpacity = Math.min(timeScale, 1);
						} else {
							let mag = props.mag;
							let magScale = 1 - ((7 - mag) / 10);
							if (magScale > 0.5) iconOpacity = magScale;
						}

						// Create earthquake icon as divIcon with SVG
						const quakeIcon = L.divIcon({
							className: 'earthquake-marker',
							html: `<svg xmlns='http://www.w3.org/2000/svg' width='40' height='40' viewBox='0 0 302.836 302.836' style='opacity:${iconOpacity}'>
								<circle cx='151' cy='151' r='15' fill='${alertColor}'/>
								<circle cx='151' cy='151' r='116' fill='none' stroke='${alertColor}' stroke-width='3' opacity='0.55'/>
								<circle cx='151' cy='151' r='42' fill='none' stroke='${alertColor}' stroke-width='3'/>
								<circle cx='151' cy='151' r='66' fill='none' stroke='${alertColor}' stroke-width='6'/>
								<circle cx='151' cy='151' r='90' fill='none' stroke='${alertColor}' stroke-width='4'/>
								<circle cx='151' cy='151' r='146' fill='none' stroke='${alertColor}' stroke-width='2' opacity='0.36'/>
							</svg>`,
							iconSize: [40, 40],
							iconAnchor: [20, 20]
						});
					...
					onEachFeature: function(feature, layer) {
						...
						if (props.alert != null) {
							alertLabelColor = props.alert == "green" ? "lawngreen" : props.alert;
						...
						const popupContent = `
							<div style="line-height:1.35;overflow:hidden;white-space:nowrap;color:black;">
								...
								${props.place}<br/><br/>
								Current <a href="https://earthquake.usgs.gov/data/pager/onepager.php" target="_blank">USGS PAGER</a> Alert Level:
								<strong style="color: ${alertLabelColor}; background-color: black; padding: 5px; border-radius: 7px; font-size: 0.9em;">${alertLabel}</strong><br/><br/>
								...
								<a href="${props.url}" target="_blank">Earthquake details</a>
							</div>`;
...
function resetZoom() {
	...
	// Reset the tilt & heading back to their original values (Leaflet doesn't support tilt/heading)...
