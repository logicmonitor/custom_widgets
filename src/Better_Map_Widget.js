	...
	if (weatherCheckbox.checked) {
		...
		// Look to see if we should add power outages to the map...
			...
			parent.overlayInfoWindowListenerHandle = map.data.addListener('click', async function(event) {
						...
						<div style="margin: 12px 0; display: flex; flex-direction: column; gap: 12px; min-width: 220px;">
							...
							<div style="display: flex; flex-direction: column; gap: 4px;">
								...
								<div style="position: relative; height: 24px;">
									...
									<div style="position: absolute; top: 5px; left: calc(${Math.min(100, Math.max(0, event.feature.getProperty("mag") / 8 * 100))}% - 12px); width: 14px; height: 12px; background: #333; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: 1px solid white;">
				...
				// Fetch and display DYFI (Did You Feel It?) intensity polygons...
				// Clear any previous intensity polygons...
				if (parent.dyfiPolygons) {
					parent.dyfiPolygons.forEach(polygon => polygon.setMap(null));
					parent.dyfiPolygons = [];
				}
				parent.dyfiPolygons = [];

				// Fetch the detail URL to get DYFI product data...
				const detailUrl = event.feature.getProperty("detail");
				if (detailUrl) {
					try {
						const detailResponse = await fetch(detailUrl);
						const detailData = await detailResponse.json();
						// Look for DYFI product with dyfi_geo.geojson...
						const dyfiProduct = detailData?.properties?.products?.dyfi?.[0];
						const dyfiGeoUrl = dyfiProduct?.contents?.["dyfi_geo.geojson"]?.url;
						
						if (dyfiGeoUrl) {
							const dyfiResponse = await fetch(dyfiGeoUrl);
							const dyfiData = await dyfiResponse.json();
							
							// Find max distance for opacity scaling...
							let maxDist = 0;
							dyfiData.features.forEach(feature => {
								if (feature.properties.dist > maxDist) maxDist = feature.properties.dist;
							});
							
							// Add each polygon to the map with opacity based on dist...
							dyfiData.features.forEach(feature => {
								if (feature.geometry && feature.geometry.type === "Polygon") {
									const coords = feature.geometry.coordinates[0].map(coord => ({lat: coord[1], lng: coord[0]}));
									const dist = feature.properties.dist || 0;
									const cdi = feature.properties.cdi || 0;
									// Calculate opacity: lower dist = more opaque (dist 0 = 0.8, max dist = 0.1)
									const opacity = Math.max(0.1, 0.8 - (dist / maxDist) * 0.7);
									// Color based on CDI intensity (MMI scale colors)
									let fillColor = '#EEE';
									if (cdi >= 8) fillColor = '#F44336';
									else if (cdi >= 6) fillColor = '#FF9800';
									else if (cdi >= 4) fillColor = '#FFEB3B';
									else if (cdi >= 2) fillColor = '#ACD8E9';
									...
									parent.dyfiPolygons.push(polygon);
								}
							});
							console.debug(`Loaded ${parent.dyfiPolygons.length} DYFI intensity polygons`);
						}
					} catch (err) {
						console.debug("Could not load DYFI data:", err.message);
					}
				}
			});
			if (showWildfireInfoEvent == "mouseover") {
